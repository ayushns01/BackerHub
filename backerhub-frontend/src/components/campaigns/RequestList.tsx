...
export default function RequestList({ campaignAddress, requestsCount }: RequestListProps) {
  ...
  function handleApprove(requestIndex: number) {
  ...
  function handleFinalize(requestIndex: number) {
    (writeContract as any)({
      address: campaignAddress as `0x${string}`,
      abi: Campaign.abi,
...
function RequestItem({ 
  ...
  const { data: request, isLoading: requestLoading } = useReadContract({
    ...
    args: [requestIndex],
  }) as { data: Request | undefined; isLoading: boolean };

  const { data: summary } = useReadContract({
    address: campaignAddress as `0x${string}`,
    abi: Campaign.abi,
    functionName: "getSummary",
  }) as { data: [string, bigint, bigint, bigint, bigint, string, string] | undefined };
